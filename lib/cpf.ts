...
export function getEffectiveBasicHealthcareSum(age: number): number {
  ...
  // For now we preserve the same modeled cap after 65 because the planner
  // only supports the default 2026 rule set and does not capture cohort-specific
  // grandfathered BHS values.
  return CPF_2026_BASIC_HEALTHCARE_SUM_UNDER_65;
}
...
export function calculateCpfMonthlyContribution(
  ...
  const maContribution = roundCurrency(
    totalContribution * allocationBand.maRatio
  );
...
export function projectCpfBalances(
  ...
  for (let month = 0; month < monthsProjected; month += 1) {
    ...
    const contribution = calculateCpfMonthlyContribution(
      age,
    ...
    const maContributionApplied = Math.min(
      contribution.maContribution,
      maHeadroom
    );
    const maOverflow = roundCurrency(
      contribution.maContribution - maContributionApplied
    );
...
export function buildCpfProjectionSummary(
  ...
  const earlyRetirementProjection = projectCpfBalances(
    startingBalances,
    settings,
