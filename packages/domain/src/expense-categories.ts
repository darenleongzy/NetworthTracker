...
export function getSubcategoriesForCategory(category: ExpenseCategory) {
  return EXPENSE_SUBCATEGORIES.filter((subcategory) => subcategory.category === category);
}

export function getSubcategoryLabel(subcategory: ExpenseSubcategory): string {
  const found = EXPENSE_SUBCATEGORIES.find((entry) => entry.value === subcategory);
  return found?.label ?? subcategory;
}

export function getCategoryLabel(category: ExpenseCategory): string {
  const found = EXPENSE_CATEGORIES.find((entry) => entry.value === category);
